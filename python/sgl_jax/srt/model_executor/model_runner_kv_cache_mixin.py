...
def _enforce_recurrent_state_server_constraints(server_args, is_lightning: bool = False) -> None:
    ...
    # HiCache host offload is unsupported with recurrent radix caching: recurrent
    # state is device-only (freed at eviction, never backed up), so the recurrent
    # match validator stops at every tombstone and the host tier can never serve
    # a hit — D2H backups and host pool RAM would be pure overhead on an
...
class ModelRunnerKVCacheMixin:
    ...
    def _resolve_max_num_reqs(self: ModelRunner, max_num_reqs: int | None) -> int:
        ...
        if (
            ...
            # A running request consumes request_owned_slots recurrent slots (1
            # running + ping-pong track slots under extra-buffer). A pool with
            # fewer slots/rank than that can never allocate even one request -- the
            # scheduler backpressure would mark every rank full forever. Fail fast
            # at startup with an actionable message instead of a stuck server.
