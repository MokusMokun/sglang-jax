...
class LogitsProcessor(nnx.Module):
    ...
    def _select_input_token_logprobs(
        ...
        # token_ids is None on batch paths that set return_logprob=True but
        # don't carry per-token gather ids (e.g. DP-padding rows from
        # get_model_worker_batch); downstream guards on
        # `output.input_token_logprobs is not None`, so propagate None.
