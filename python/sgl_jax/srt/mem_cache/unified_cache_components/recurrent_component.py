...
Scope (page_size=1): finished-request commit + prefix-hit copy-on-write
clone. Unfinished/fork donation and page_size>=128 support are deferred.
"""
...
class RecurrentComponent(TreeComponent):
    ...
    def prepare_for_caching_req(
        ...
        # materialized state, so the tree value is the slot itself (no copy).
        # Unfinished/fork donation is deferred to avoid the
        # publish-before-materialize race.
    ...
    def cleanup_after_caching_req(
        ...
        # donate nothing and keep their running slot (the live state);
