...
class PrefillAdder:
    ...
    def add_chunked_req(self, req: Req):
        ...
        # Return if chunked prefill not finished (chunk-budget OR boundary split).
    ...
    def add_one_req(self, req: Req):
        ...
        with self._lock_node(req.last_node):
            ...
            if chunk_budget_ok and not boundary_truncated:
                ...
                # Boundary-only split: chunk budget had room, but the EXTEND was
                # capped to a track boundary; commit it as a chunked req.
                self.can_run_list[dp_rank].append(req)
                self.new_chunked_reqs[dp_rank] = req
                res = self.tree_cache.inc_lock_ref(req.last_node)
                req.swa_uuid_for_lock = res.swa_uuid_for_lock
                ...
                # Chunked prefill (chunk budget tighter than the boundary cap when
                # both apply, so this is the minimum of the two caps).
                req.extend_input_len = trunc_len
